...
function fetchDone(accept, acceptTypeOnError, response) {
    function parseResponse(accept, response, resolve, reject) {
        switch (accept) {
            case 'text':
        ...
        if (response.ok) {
            if (response.url.indexOf(USER_SERVICE_BASEPATH+'/user/api/login') === 0) {
                setCookies(response.headers.getAll('set-cookie'), cookies => {
                    parseResponse(accept, response, resolve, reject);
                });
                return;
            ...
            parseResponse(acceptTypeOnError||'json', response, reject, reject);
        }
    })
}
...
function fetchSend(url, params, option, resolve, reject) {
    ...
    console.log(params)
    fetch(url, params)
        .then(fetchDone.bind(null, option.acceptType, option.acceptTypeOnError))
        .then(resolve)
        .catch(reject)
}
...
function callApi(option) {
    return new Promise((resolve, reject) => {
        ...
        var accept = acceptTypes[option.acceptType||'json'];
        var params = {
            method: method,
            headers: {}
        }
