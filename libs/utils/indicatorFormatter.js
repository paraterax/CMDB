/**
    ...
    @example indicatorFormatter("number_k")(32432);
*/
'use strict';

const NaN_Str = 'N/A';
function isNone(value) {
	return value === null || value === undefined || value == NaN_Str;
}
	...
	switch(type){
		...
		case "number_thousand":
			return function(val){
				...
				return isNone(val)?NaN_Str:String(val).replace(reg, '$&,');
			}
		case "time":
			return function(val){
				if(isNone(val)){
		...
		case "flow_mb":
			return function(val){
				return  isNone(val)?NaN_Str:(val/1048576).toFixed(2);
			}
		case "flow_kb":
			return function(val){
				return  isNone(val)?NaN_Str:(val/1024).toFixed(2);
			}
		default:
			return function(val){
				return isNone(val)?NaN_Str:val;
			}
	}
}
