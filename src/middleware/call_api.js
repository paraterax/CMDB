 ...
 */
import { encodeQueryParam } from 'libs/utils/queryParam';
import { SERVICE_BASEPATH } from 'app/constants/api'
 ...
 * 请求响应处理函数
 */
function fetchDone(accept, acceptTypeOnError, response) {
    ...
    return new Promise((resolve, reject) => {
        if (response.ok) {
            ...
            if(response.status == "401"){
                parent.window.location.href = SERVICE_BASEPATH + "/login.html";
            }else{
 ...
 * @param {string} [option.acceptType] 返回的数据格式, 默认为json, 可选的值有：json|text|stream
 ...
 * @param {string} [option.acceptTypeOnError] 请求异常时返回的数据格式, 默认为json, 可选的值有：json|text
 */
var isStart = false;
var socket = null;
...
function callApi(option) {
    ...
    var acceptType = option.acceptType || 'json';
    var acceptTypeOnError = option.acceptTypeOnError || 'json';
    var headers = option.headers || null;
    var accept = acceptTypes[acceptType];
    var params = {
        method: method,
        headers: {},
        mode: 'cors',
    ...
    let connectMode = option.connectMode || "http"
    if (accept) {
